...
const deviceType = {
    ...
    minimumTabletMedia: window.matchMedia('(min-width: 768px)'),
};
...
function fadeToggle(
    el,
    { speed = 160, display = 'block', toggleClass = '' } = {}
) {
    if (isHidden(el)) {
 ...
 */
function slideDown(el, { speed = 120, display = 'block', toggleClass = '' } = {}) {
 ...
 * @returns {slideFunctionReturn}
 */
function slideUp(el, { speed = 120, toggleClass = '' } = {}) {
    let resolve, reject, cancelled;
    const promise = new Promise((promiseResolve, promiseReject) => {
        resolve = promiseResolve;
        reject = promiseReject;
 ...
 * @param {String} [props.toggleClass = ''] - classList to toggle on animation end
 */
function slideToggle(el, { speed = 120, display = 'block', toggleClass = '' } = {}) {
    if (isHidden(el)) {
        return slideDown(el, { speed, display, toggleClass });
    } else {
        return slideUp(el, { speed, toggleClass });
    }
}
